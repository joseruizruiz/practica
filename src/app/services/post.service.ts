...
export class PostService {
    ...
    getUserPosts(id: number): Observable<Post[]> {
        ...
        const x = moment();
        return this._http
            .get(`${this._backendUri}/posts?publicationDate_lte=${x}&_sort=publicationDate&_order=DESC&author.id=${id}`)
            .map((response: Response) => Post.fromJsonToList(response.json()));
    }
    ...
    getCategoryPosts(id: number): Observable<Post[]> {
        ...
        return this._http
            .get(`${this._backendUri}/posts?_sort=publicationDate&_order=DESC&publicationDate_lte=${x}`)

            .map((response: Response) => {
                return Post.fromJsonToList(response.json()).filter((post: Post) => {
                    //console.log(post);
                    var cat = post.categories.findIndex((cat: Category) => {
